...
v5G_start <v5G_name> <vnf_tunnel_ip> <home_tunnel_ip> <v5G_private_ip> <v5G_public_ip> <dhcpd_conf_file>
    being:
        ...
        <v5G_private_ip>: the private ip address for the 5G Control Plane
        <v5G_public_ip>: the public ip address for the External network (10.2.2.0/24)
        <dhcpd_conf_file>: the dhcp file for the 5G network to give private addresses to the home network
"
...
echo "--"
echo "--Connecting v5G service with AccessNet and ExtNet with the UPF..."
...
## 1. En VNF:vupf1 agregar un bridge y asociar interfaces.
...
## 2. En VNF:vupf2 agregar un bridge y asociar interfaces.
sudo docker exec -it $VNF2 ovs-vsctl add-br br1
sudo docker exec -it $VNF2 /sbin/ifconfig br1 $V5GPRIVIP/24
...
## 3. En VNF:upf2 asignar dirección IP a interfaz de salida.
sudo docker exec -it $VNF2 /sbin/ifconfig veth0 $V5GPUBIP/24
